...
自作の[電池長期間駆動Arduinoロガー](https://citriena.seesaa.net/article/202001article_2.html)で使っているスケッチです。サーミスタであれば、1分毎測定、10分毎記録で1年間以上連続稼働が可能です。LCDを切れば電池交換無しで数年は稼働可能です。

![電池長期間駆動Arduinoロガー](https://citriena.up.seesaa.net/04367611N000000000/157995619336870194040-thumbnail2.jpg "外観")
![電池長期間駆動Arduinoロガー](https://citriena.up.seesaa.net/04367611N000000000/157995623933730891559-thumbnail2.jpg "内部")
...
### 対応済みセンサー（ライブラリ）
* 現在対応済みセンサーのライブラリは、以下のsensorSHT.h（Sensirion SHT-21/25, SHT-31/35等用）、sensorBME280.h（Bosch BME280用），およびsensorNTC.h（サーミスタ用） の３つです。CO2センサ（MHZ19, SCD40）もセンサーライブラリは作成していますがテストしていません。それ以外のセンサーを使う場合はセンサライブラリの作成が必要です。異なるセンサの組み合わせも可能です。
...
EEPROMメモリを一周したら、上書きされたヘッダ単位で前のデータが順に読み出せなくなります。
...
1. 取得したデータは一旦Arduino内のメモリに確保したバッファ（複数データ保存可能　上記の例ではEM_DATA_PER_BUFF = 4）に保存
...
|  |
## RS-274Xデータについて
Fritzingで作成したプリント基板データをRS-274X形式で出力し、製造委託先（Elecrow）に合わせてファイル名を変更したものです。他の会社では使えないかもしれません。
...
* 再コンパイルしなくてもロギング条件の設定を可能にする。FLASHメモリに余裕がない上にボタン１個での操作なのでちょっと面倒
...
### 1.2.2 - Oct. 28, 2021 ENDLESSモードで２周目先頭がヘッダの時のデータ読出し漏れ fix
