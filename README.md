...
まだ開発途上ですが、センサの設定によっては現在でもFLASHメモリ使用量が90%を超えます。ロギング条件設定機能を実装できるか微妙なところです。いずれコードを見直してメモリ節約したいところです。少なくともDUAL_SENSORSについては配列とループでの処理に変更予定です。
...
#### 機能はしたが、センサや機能の設定用#ifdefが多すぎる暫定版
